...
impl SignalBus {
    ...
    /// Adjust a signal's per-tick decay rate. Clamped to [0.0, 0.5].
    pub fn set_decay_rate(&mut self, id: SignalId, rate: f64) {
        if let Some(sig) = self.signals.get_mut(&id) {
            sig.decay_rate = rate.clamp(0.0, 0.5);
        }
    }

    /// Shift a signal's baseline. Clamped to [0.0, 1.0].
    pub fn set_baseline(&mut self, id: SignalId, baseline: f64) {
        if let Some(sig) = self.signals.get_mut(&id) {
            sig.baseline = baseline.clamp(0.0, 1.0);
        }
    }
}
