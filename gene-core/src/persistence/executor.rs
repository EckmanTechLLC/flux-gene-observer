...
impl SystemOpExecutor {
    ...
    pub fn execute(
        ...
        match op {
            // Financial-gene perception meta-actions — handled by financial executor, not here
            SystemOp::AdjustDecay { .. }
            | SystemOp::AdjustBaseline { .. }
            | SystemOp::AdjustWeight { .. }
            | SystemOp::CoinDerivedSignal { .. } => (OpResult {
                success: false,
                output: "financial op not dispatched in os executor".into(),
                signal_feedback: vec![],
            }, None),
