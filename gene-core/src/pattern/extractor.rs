...
impl PatternExtractor {
    ...
    /// Returns signals deviating from their baseline by more than their per-signal threshold.
    ...
    ///
    /// `thresholds` — per-signal adaptive thresholds (e.g. from StddevTracker).
    /// `default_threshold` — fallback for signals absent from `thresholds`.
    pub fn current_active(
        &self,
        thresholds: &HashMap<SignalId, f64>,
        baselines: &HashMap<SignalId, f64>,
        default_threshold: f64,
    ) -> HashMap<SignalId, f64> {
        if let Some(snap) = self.window.last() {
            snap.values.iter()
                .filter_map(|(id, val)| {
                    let baseline  = baselines.get(id).copied().unwrap_or(*val);
                    let dev       = (val - baseline).abs();
                    let threshold = thresholds.get(id).copied().unwrap_or(default_threshold);
                    if dev > threshold { Some((*id, dev)) } else { None }
                })
                .collect()
        } else {
            HashMap::new()
